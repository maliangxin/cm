...
public class BanktofundpaymentRule extends AbstractCommonRule {

    private final String FundPayment_b = "FundPayment_b";
    ...
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> paramMap) throws Exception {
        ...
        while (iterator.hasNext()) {
            ...
            if (null == map.get(FundPayment_b)) {
                throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102196"), com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_17FE8C540418025F", "推单映射异常！") /* "推单映射异常！" */);
            }
            ...
            if (paramMap.containsKey("externalData") && paramMap.get("externalData") != null) {
                ...
                if (externalData.containsKey("transtypeId") && externalData.get("transtypeId") != null) {
                    ...
                    if (!org.apache.commons.collections4.CollectionUtils.isEmpty(transTypes)) {
                        ...
                        //交易类型为委托付款不支持生单
                        if ("cmp_fund_payment_delegation".equals(bdTransType.getCode())) {
                            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102197"), com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_1A0AD2B605B80001", "该交易类型不支持生单！") /* "该交易类型不支持生单！" */);
                        }
            ...
            if (map.get("transtype_publish_service_code") == null || StringUtils.isEmpty((CharSequence) map.get("transtype_publish_service_code"))) {
                map.put("transtype_publish_service_code", IServicecodeConstant.FUNDPAYMENT);
            }

            String accentity = map.get(ACCENTITY).toString();
            String currency = map.get(IBussinessConstant.CURRENCY).toString();
            ...
            if (defaultExchangeRateType == null || defaultExchangeRateType.getId() == null) {
                throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102198"), com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:YS_CTM_CM-BE_LOCAL_00050033", "此资金组织对应会计主体下无默认汇率类型，请检查数据！") /* "此资金组织对应会计主体下无默认汇率类型，请检查数据！" */);
            }
            ...
            if (!StringUtils.isEmpty(accentity) && !StringUtils.isEmpty(currency)) {
                    ...
                    try {
                        ...
                        if (cmpExchangeRateVO == null || cmpExchangeRateVO.getExchangeRate() == BigDecimal.ZERO) {
                            throw new CtmException(String.format(IMultilangConstant.noRateStringError /* "未获取到汇率类型为[%s]的[%s]到[%s]的汇率值，请检查汇率配置！" */,
                                    map.get(EXCHANGERATETYPENAME), oriCurrencyName, map.get("natCurrency_name")));
                        }
                ...
                map.put(EXCHRATEOPS, exchangeRateOps);
            }
            ...
            List<Map<String, Object>> purchaseOrders = (List) map.get(FundPayment_b);
            ...
            while (iter.hasNext()) {
                ...
                //结算方式
                if (!CollectionUtils.isEmpty(dataList) && !ValueUtils.isNotEmptyObj(map_sub.get(SETTLE_MODE))) {
                ...
                //汇率类型-
                map_sub.put(EXCHANGERATETYPE, defaultExchangeRateType.getId());
                ...
                map_sub.put(EXCHRATEOPS, exchangeRateOps);
                ...
                creditAmountMain = BigDecimalUtils.safeAdd(creditAmountMain, (BigDecimal) map_sub.get(NATSUM));
                    ...
                    if (!CollectionUtils.isEmpty(sourceDatas)) {
                        for (Map<String, Object> sourceData : sourceDatas) {
                            if (null != sourceData && null != sourceData.get("code") && !StringUtils.isEmpty(sourceData.get("code").toString())) {
                                code.append(sourceData.get("code").toString())
                                        .append(",");
                            }
                        }
                    ...
                    if (!CollectionUtils.isEmpty(sourceDatas)) {
                        for (Map<String, Object> sourceData : sourceDatas) {
                            if (null != sourceData && null != sourceData.get("bank_seq_no") && !StringUtils.isEmpty(sourceData.get("bank_seq_no").toString())) {
                                code.append(sourceData.get("bank_seq_no").toString())
                                        .append(",");
                            }
                        }
                        if (StringUtils.isNotEmpty(code.toString())) {
    ...
    private void executeSaveBusinessLog(Map<String, Object> paramMap, StringBuilder code, String serviceCode, String name) {
        try {
            ...
            ctmcmpBusinessLogService.saveBusinessLog(map, code.toString(), "", serviceCode, name, IMsgConstant.CMDFUNDPAYMENT);
        } catch (Exception e) {
            log.error("BanktofundpaymentRule ctmcmpBusinessLogService：", e);
        }
    }
