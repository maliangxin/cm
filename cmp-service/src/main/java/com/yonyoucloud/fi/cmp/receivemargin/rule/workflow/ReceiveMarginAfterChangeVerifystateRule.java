 ...
 * *
 */
@Component("receiveMarginAfterChangeVerifystateRule")
@Slf4j
@RequiredArgsConstructor
public class ReceiveMarginAfterChangeVerifystateRule extends AbstractCommonRule {
    @Autowired
    ...
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> map) throws Exception {
        ...
        for (BizObject bizobject : bills) {
            ReceiveMargin receiveMargin = (ReceiveMargin) MetaDaoHelper.findById(fullName, bizobject.getId(), ICmpConstant.CONSTANT_TWO);
            //驳回/撤回到发起人/审批终止
            //是否占预算为预占成功时，删除预占
            if (BooleanUtils.b(receiveMargin.get(ICmpConstant.IS_WFCONTROLLED)) && ("REJECTTOSTART".equals(deleteReason) || "ACTIVITI_DELETED".equals(deleteReason))) {
                if (cmpBudgetManagerService.isCanStart(IBillNumConstant.CMP_RECEIVEMARGIN)) {
                    if (receiveMargin.getIsOccupyBudget() != null && OccupyBudget.PreSuccess.getValue() == receiveMargin.getIsOccupyBudget()) {
                        cmpBudgetReceivemarginManagerService.executeSubmitDelete(receiveMargin);
                        //刷新pubts
            ...
            } else if (BooleanUtils.b(receiveMargin.get(ICmpConstant.IS_WFCONTROLLED)) && ("WITHDRAWREJECTTOSTART".equals(deleteReason))) {
            ...
            } else if (BooleanUtils.b(receiveMargin.get(ICmpConstant.IS_WFCONTROLLED)) && ("withdraw".equals(deleteReason))) {
                log.error("withdraw........");
            }
        }
