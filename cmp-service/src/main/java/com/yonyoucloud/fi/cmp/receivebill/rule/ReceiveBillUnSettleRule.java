 ...
 */
@Component
public class ReceiveBillUnSettleRule extends AbstractCommonRule{
    ...
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> map) throws Exception {
        if(CmpCommonUtil.getNewFiFlag()){
            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-103033"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_20C2621A04A00006", "在财务新架构环境下，不允许取消结算收款单。") /* "在财务新架构环境下，不允许取消结算收款单。" */);
        }
        List<ReceiveBill> receiveBillList =  this.getBills(billContext, map);
        ReceiveBill currentReceiveBill = receiveBillList.get(0);
        cmpReceiveBillSettleServiceImpl.unsettle(currentReceiveBill);
