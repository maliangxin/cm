...
@Slf4j
@Component
public class PayMarginBeforeSaveRule extends AbstractCommonRule {

    public static final String OLD_PAYMARGIN = "OLD_PAYMARGIN";
    ...
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> map) throws Exception {
        ...
        for (BizObject bizObject : bills) {
            ...
            if (ObjectUtils.isNotEmpty(payMargin.getId())) {
                PayMargin oldPayMargin = MetaDaoHelper.findById(PayMargin.ENTITY_NAME, payMargin.getId(), 1);
                payMargin.put(OLD_PAYMARGIN, oldPayMargin);
            }
            checkPayMarginVirtualAccountBalance(payMargin);
        }
        return new RuleExecuteResult();
    }

    private void checkPayMarginVirtualAccountBalance(PayMargin payMargin) throws Exception {
        //校验保证金虚拟户的可用余额
        ...
        //2.转换的需要校验转换金额+取回金额
        if (transType.getCode().equals("cmp_paymargin_withdraw") || paymargin_ext.equals("cmp_paymargin_withdraw")) {
            ...
            if (payMargin.getEntityStatus() == EntityStatus.Insert) {
                marginamount = payMargin.getMarginamount();
                conversionamount = payMargin.getConversionamount();
            } else if (payMargin.getEntityStatus() == EntityStatus.Update) {
                PayMargin payMargin_old = MetaDaoHelper.findById(PayMargin.ENTITY_NAME, payMargin.getId(), 1);
                //保存前金额
                BigDecimal marginamount_old = payMargin_old.getMarginamount();
                marginamount = payMargin.getMarginamount().subtract(marginamount_old);
                if (payMargin_old.getConversionmarginflag() == 1 && payMargin.getConversionmarginflag() == 1) {
                    BigDecimal conversionamount_old = payMargin_old.getConversionamount();
                    conversionamount = payMargin.getConversionamount().subtract(conversionamount_old);
                } else if (payMargin_old.getConversionmarginflag() == 0 && payMargin.getConversionmarginflag() == 1) {
                    conversionamount = payMargin.getConversionamount();
                }
            }
            ...
            if (ObjectUtils.isEmpty(marginWorkbench)) {
                throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102087"), com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_1CA6189A04780006", "取回保证金的时候未查询到虚拟户信息，请检查！") /* "取回保证金的时候未查询到虚拟户信息，请检查！" */);
            }
            //针对虚拟户上锁，直到事务结束，避免并发
            ...
            //校验金额
            log.error("扣减的虚拟金账户id:{}, 虚拟金可用余额：{}", marginWorkbench.getId(), marginWorkbench.getMarginAvailableBalance());
