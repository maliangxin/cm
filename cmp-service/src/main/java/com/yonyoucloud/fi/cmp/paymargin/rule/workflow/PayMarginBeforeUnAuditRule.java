...
@Slf4j
@Component
public class PayMarginBeforeUnAuditRule extends AbstractCommonRule {
    ...
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> map) throws Exception {
        ...
        for (BizObject bizobject : bills) {
            ...
            try {
                ...
                log.info("PayMarginUnAuditRule bizObject, id = {}, pubTs = {}", bizobject.getId(), bizobject.getPubts());
                PayMargin currentBill = MetaDaoHelper.findById(PayMargin.ENTITY_NAME, bizobject.getId());
                log.info("PayMarginUnAuditRule currentBill, id = {}, pubTs = {}", currentBill.getId(), currentBill.getPubts());
                if (currentBill == null) {
                ...
                if (transType.getCode().equals("cmp_paymargin_payment") || paymargin_ext.equals("cmp_paymargin_payment")) {
                    ...
                    if (ObjectUtils.isEmpty(marginWorkbench)) {
                        throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-100169"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_1CABCB6404380003", "支付保证金撤回，未查询到虚拟户信息，请检查！") /* "支付保证金撤回，未查询到虚拟户信息，请检查！" */);
                    }
                    //针对虚拟户上锁，直到事务结束，避免并发
...
//                // 结算状态为已结算补单 并且 推送结算 并且 凭证状态是未生成的  不允许撤回
...
//                        throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-100171"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_1A6FF92405880003", "结算状态为已结算补单并且推送结算为是并且凭证状态是未生成,不能进行撤回！") /* "结算状态为已结算补单并且推送结算为是并且凭证状态是未生成,不能进行撤回！" */);
//                    }
//                }
                ...
                short settlestatus = currentBill.getSettlestatus();
                short settleflag = currentBill.getSettleflag();
                short samenametransferflag = currentBill.getSamenametransferflag();
                String ourbankaccount = currentBill.getOurbankaccount();
                if (ObjectUtils.isNotEmpty(settlestatus)) {
                    if (settlestatus == FundSettleStatus.SettlementSupplement.getValue() && settleflag == 1
                        ...
                        querySettledDetailModel.setBusinessDetailsId(currentBill.getId().toString());//业务单据明细ID -> 转账单-ID
                ...
                // 结算状态为已结算补单 并且 推送结算 并且 凭证状态是未生成的不允许撤回
                short voucherstatus = currentBill.getVoucherstatus();
                ...
                // 弃审需要校验 1.转换标识为是，弃审的时候需要删除对应的子单据（生成的转换金） 2.如果是收到保证金（receivemargin）校验退还标识 如果存在退换单 不允许弃审
                if (currentBill.getConversionmarginflag() == 1) {
                    if (ObjectUtils.isEmpty(currentBill.getConversionmarginid())) {
                        ...
                        PayMargin conversionPayMargin = MetaDaoHelper.findById(PayMargin.ENTITY_NAME, conversionmarginid);
                        conversionPayMargin.put(ENTITYNAME, PayMargin.ENTITY_NAME);
                        // 如果有转换保证金，需要删除对应的转换保证金的凭证
                        CtmJSONObject deleteResult = cmpVoucherService.deleteVoucherWithResult(conversionPayMargin);
                        ...
                        params.put(ICmpConstant.MARGINWORKBENCH_ID, conversionPayMargin.getMarginvirtualaccount());
                        params.put(ICmpConstant.PAYMARGIN, conversionPayMargin);
                        //todo 调用工作台删除接口 ：如果只有此单据占用了此虚拟户 调用删除虚拟户接口 否则 不调用
                        Boolean deleteflag = marginCommonService.useByMulPayMargin(conversionPayMargin.getMarginvirtualaccount().toString());
                        //根据id删除
                        MetaDaoHelper.delete(PayMargin.ENTITY_NAME, conversionmarginid);
                        //调用工作台更新接口
                    ...
                    if (ObjectUtils.isNotEmpty(currentBill.getConversionmarginid())) {
                        throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-100174"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_18EC6E4E05080001", "转换生成的支付保证金台账，无法弃审！") /* "转换生成的支付保证金台账，无法弃审！" */);
                    }
                ...
                throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-100175"),e.getMessage());
            } finally {
