...
@Slf4j
public class BankreconciliationRuleToReceive extends AbstractCommonRule {
    @Autowired
    ...
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> paramMap) throws Exception {
        ...
        if (CollectionUtils.isEmpty(omakes)) {
            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102286"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_17FE8C540418073E","所选的生单数据信息不完整，请检查数据必填项是否已填") /* "所选的生单数据信息不完整，请检查数据必填项是否已填" */);
        }
    ...
    private void assembleInfo(Map<String, Object> map) throws Exception {
        ...
        //收款对象---当前会计主体以及对应的业务组织的客户档案进行匹配，如果匹配到对应客户，则收付款对象类型=客户；如果没有匹配，则再在供应商档案进行匹配，如果匹配对应供应商，则收付款对象类型=供应商，如果仍未匹配，则收付款对象类型=其他；
        ...
        Map<String, Object> queryTransType = cmCommonService.queryTransTypeById("FICM1", "1", null);
        ...
        if (null == toAcctName || null == toAcctNo) {
            ...
            //客户供应商信息
            getReceiveBillType(map, bankReconciliation);
        }

        //本币币种
        List<Map<String, Object>> accEntityList = QueryBaseDocUtils.queryAccRawEntityByAccEntityId(accentity);
        if (accEntityList.size() != 0) {
            CurrencyTenantDTO currencyTenantDTO = baseRefRpcService.queryCurrencyById(accEntityList.get(0).get("currency").toString());
        ...
        if (query != null && query.size() > 0 && query.get(0).get("receiveQuickType") != null) {
            condition.put("id", query.get(0).get("receiveQuickType"));
            ...
            if (ValueUtils.isNotEmpty(payQuickType)) {
                ...
                if("1".equals(quickType_code) || "2" .equals(quickType_code) || "5".equals(quickType_code) || "6".equals(quickType_code)){
                    throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102287"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_17FE8C540418073C","款项类型默认值不能为应收应付，请修改款项类型！") /* "款项类型默认值不能为应收应付，请修改款项类型！" */);
                }
        ...
        if (defaultExchangeRateType.isEmpty() || defaultExchangeRateType.get("id") == null) {
            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102288"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:YS_CTM_CM-BE_LOCAL_00050034", "此资金组织对应会计主体下无默认汇率类型，请检查数据！") /* "此资金组织对应会计主体下无默认汇率类型，请检查数据！" */);
        }
        ...
        //收款金额
        map.put(IBussinessConstant.ORI_SUM, map.get("creditamount"));
        //汇率
        getExchRate(map, bankReconciliation, defaultExchangeRateType);
        //余额
        ...
        if (maxSettleDate != null) {
            if (maxSettleDate.compareTo(vouchdate) >= 0) {
                throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102289"),com.yonyou.iuap.ucf.common.i18n.MessageUtils.getMessage("P_YS_FI_CM_0000153370") /* "单据日期已日结，不能保存单据！" */);
            }
        }
        ...
        if (CollectionUtils.isEmpty(autoConfigList)) {
            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102290"),MessageUtils.getMessage("P_YS_CTM_CM-BE_0001587833") /* "未配置现金参数，请配置" */);
        }
        if (autoConfigList.get(0).get("settlemode") == null) {
            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102291"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_17FE8C5404180741","自动化参数未配置结算方式") /* "自动化参数未配置结算方式" */);
        }
     ...
     * @throws Exception
     */
    private void getReceiveBillType(Map<String, Object> map, BankReconciliation bankReconciliation) throws Exception {
        Object toAcctName = map.get("to_acct_name");
...
//            map.put("operator_name", supplierJson.get("iPersonIdName"));
            ...
            if (Objects.equals(MerchantConstant.TRUE, customerJson.getString(MerchantConstant.CUSTOMERFLAG))) {
                ...
                map.put(IBussinessConstant.CUSTOMER, customerId);
...
//                map.put("operator_name", customerJson.get("cProfessSalesmanName"));
                ...
                Map<String, Object> employee = getEmployee(toAcctName, map.get("to_acct_no"), map.get(IBussinessConstant.ACCENTITY));
    ...
    private Map<String,Object> getEmployee(Object toAcctName, Object toAcctNo, Object accentity) throws Exception {
        ...
        querySchema_period.addSelect(
                "bankAcctList.id as staffBankAccount, bankAcctList.account as staffBankAccount_account, " +
                        ...
                        "mainJobList.org_id as org, mainJobList.org_id.name as org_name");        // 对应的员工姓名、银行账号
     ...
     * @throws Exception
     */
    private void getExchRate(Map<String, Object> map, BankReconciliation bankReconciliation, Map<String, Object> defaultExchangeRateType) throws Exception {
        String accentityes = map.get(IBussinessConstant.ACCENTITY).toString();
        String currencyes = map.get("currency").toString();
        String oriCurrencyName = AppContext.getBean(CurrencyQueryService.class).findById(currencyes).getName();
        //本币金额
            ...
            if (orgCurrency != null) {
                if (currencyes.equals(orgCurrency)) {
                    ...
                    if (defaultExchangeRateType != null && defaultExchangeRateType.get("id") != null) {
                        try {
                            Double currencyRateNew = CurrencyUtil.getCurrencyRateNew(null, defaultExchangeRateType.get("id").toString(), currencyes, orgCurrency, bankReconciliation.getTran_date(), 6);
    ...
    private static CtmJSONObject getCustomer(BankReconciliation bankReconciliation) throws Exception {
        ...
        if (Objects.equals(MerchantConstant.TRUE, cust2Check.getString(MerchantConstant.CUSTOMERFLAG))) {
            if (Objects.equals(true, cust2Check.getBoolean(MerchantConstant.STOPSTATUS))) {
                throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102292"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_181197B805B00137", "当前客户档案已停用") /* "当前客户档案已停用" */);
            }
            // 客户名称和账号都存在
    ...
    private static CtmJSONObject getSupplier(BankReconciliation bankReconciliation) throws Exception {
        ...
        if (Objects.equals(MerchantConstant.TRUE, vendor2Check.getString(MerchantConstant.VENDORFLAG))) {
            if (Objects.equals(true, vendor2Check.getBoolean(MerchantConstant.STOPSTATUS))) {
                throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102293"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_181197B805B00138", "当前供应商档案已停用") /* "当前供应商档案已停用" */);
            }
            // 供应商名称和账号都存在
