...
@Component("foreignPaymentAfterDeleteBpmRule")
public class ForeignPaymentAfterDeleteBpmRule extends AbstractCommonRule {
    @Autowired
    ...
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> map) throws Exception {
        ...
        for (BizObject bizobject : bills) {
            ForeignPayment foreignPayment = (ForeignPayment) MetaDaoHelper.findById(fullName, bizobject.getId(), ICmpConstant.CONSTANT_TWO);
            //驳回/撤回到发起人/审批终止
            //是否占预算为预占成功时，删除预占
            if (BooleanUtils.b(foreignPayment.get(ICmpConstant.IS_WFCONTROLLED)) && ("REJECTTOSTART".equals(deleteReason) || "ACTIVITI_DELETED".equals(deleteReason))) {
                boolean success = cmpBudgetForeignpaymentManagerService.releaseBudget(foreignPayment);
                if (success) {
            ...
            } else if (BooleanUtils.b(foreignPayment.get(ICmpConstant.IS_WFCONTROLLED)) && ("WITHDRAWREJECTTOSTART".equals(deleteReason))) {
                log.error("WITHDRAWREJECTTOSTART........");
                boolean success = cmpBudgetForeignpaymentManagerService.budget(foreignPayment);
                if (success) {
                    cmpBudgetForeignpaymentManagerService.updateOccupyBudget(foreignPayment, OccupyBudget.PreSuccess.getValue());
                }
            } else if (BooleanUtils.b(foreignPayment.get(ICmpConstant.IS_WFCONTROLLED)) && ("withdraw".equals(deleteReason))) {
                log.error("withdraw........");
            }
        }
