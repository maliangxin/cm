...
public class CmpBudgetPaymarginManagerServiceImpl implements CmpBudgetPaymarginManagerService {
    @Value("${billdetail-url}")
    private String billDetailUrl;
    public static final String PAYMARGIN = "PayMargin__";
    @Autowired
    ...
    @Override
    public CtmJSONArray queryPaymargin(CmpBudgetEventBill budgetEventBill) throws Exception {
        //1 查询指定时间内的单据
         ...
         */
        List<PayMargin> payMargins = new ArrayList<>();
        ...
        if ("pre".equals(ruleType)) {
            ...
            List<PayMargin> list0 = MetaDaoHelper.queryObject(PayMargin.ENTITY_NAME, querySchema0, null);
            payMargins = list0;
            updatePaymargin(list0, OccupyBudget.PreSuccess.getValue());
            ...
            // 主表List集合
            List<PayMargin> list = MetaDaoHelper.queryObject(PayMargin.ENTITY_NAME, querySchema0, null);
            payMargins = list;
            updatePaymargin(list, OccupyBudget.ActualSuccess.getValue());
        }
        // 数据查询完毕，准备拼装报文
        CtmJSONArray array = new CtmJSONArray();
        HashMap<String, String> butyCode = new HashMap<>();
        for (PayMargin payMargin : payMargins) {
            CtmJSONObject payMarginJson = CtmJSONObject.parseObject(CtmJSONObject.toJSONString(payMargin));
            Map characterDefMap = payMargin.get("characterDef");
            if (characterDefMap != null && !characterDefMap.isEmpty()) {
                characterDefMap.forEach((key, value) -> payMarginJson.put(cmpBudgetCommonManagerService.CHARACTERDEF_ + key, value));
            }
            //规则id，从推送的消息中获取
            payMarginJson.put(cmpBudgetCommonManagerService.RULE_ID, budgetEventBill.getRuleId());
            payMarginJson.put(cmpBudgetCommonManagerService.RULECTRL_ID, budgetEventBill.getRuleCtrlId());
            //规则类型 (预占pre；执行implement) 从推送的消息中获取
            payMarginJson.put(cmpBudgetCommonManagerService.RULE_TYPE, budgetEventBill.getRuleType());
            //单据类型 可从推送消息中获取，也可以直接写成自己对接的单据类型
            payMarginJson.put(cmpBudgetCommonManagerService.BILL_CODE, budgetEventBill.getBillCode());
            //交易类型
            String bustype = payMargin.getTradetype();
            if (butyCode.containsKey(bustype)) {
                payMarginJson.put(cmpBudgetCommonManagerService.TRANSAC_CODE, butyCode.get(bustype));
            } else {
                String code = cmCommonService.getDefaultTransTypeCode(bustype);
                butyCode.put(bustype, code);
                payMarginJson.put(cmpBudgetCommonManagerService.TRANSAC_CODE, code);
            }
            //单据id
            payMarginJson.put(cmpBudgetCommonManagerService.BILL_ID, payMargin.getId());
            //单据编号
            payMarginJson.put(cmpBudgetCommonManagerService.BILL_BUS_CODE, payMargin.getCode());
            payMarginJson.put(cmpBudgetCommonManagerService.LINE, payMargin.getId());
            payMarginJson.put(cmpBudgetCommonManagerService.BILL_LINE_ID, payMargin.getId());
            payMarginJson.put(cmpBudgetCommonManagerService.SERVICE_CODE, IServicecodeConstant.PAYMARGIN);
            array.add(payMarginJson);
        }
    ...
    @Override
    public void updatePaymargin(List<PayMargin> paymargins, Short isOccupyBudget) throws Exception {
        if (CollectionUtils.isEmpty(paymargins)) {
            return;
        }
        List<PayMargin> updateList = new ArrayList<>();
        paymargins.stream().forEach(item -> {
        ...
        if (CollectionUtils.isNotEmpty(updateList)) {
            MetaDaoHelper.update(PayMargin.ENTITY_NAME, updateList);
        }
    }

    @Override
    @Transactional(rollbackFor = RuntimeException.class)
    public ResultBudget gcExecuteTrueAudit(BizObject bizObject, PayMargin payMargin, String billCode, String billAction, boolean deleteBudget, boolean checkResult) throws Exception {
        try {
            ...
            if (!StringUtils.isEmpty(preAction) || deleteBudget) {
                Map preBill = initPaymarginBill(bizObject, payMargin, BudgetDirect.REDUCE.getIndex(), billCode, cmpBudgetCommonManagerService.PRE, BillAction.APPROVE_PASS);
                ...
                Map implementBill = initPaymarginBill(bizObject, payMargin, BudgetDirect.ADD.getIndex(), billCode, cmpBudgetCommonManagerService.IMPLEMENT, BillAction.APPROVE_PASS);
    ...
    @Override
    @Transactional(rollbackFor = RuntimeException.class)
    public ResultBudget gcExecuteTrueUnAudit(BizObject bizObject, PayMargin payMargin, String billCode, String billAction) throws Exception {
        try {
            ...
            if (!StringUtils.isEmpty(preAction)) {
                Map<String, Object> prebill = initPaymarginBill(bizObject, payMargin, BudgetDirect.ADD.getIndex(), billCode, cmpBudgetCommonManagerService.PRE, billAction);
                controlDetailVOPre = cmpBudgetCommonManagerService.initControlDetailVO(prebill, preAction, 0);
                Map<String, Object> implementBill = initPaymarginBill(bizObject, payMargin, BudgetDirect.REDUCE.getIndex(), billCode, cmpBudgetCommonManagerService.IMPLEMENT, billAction);
    ...
    @Override
    @Transactional(rollbackFor = RuntimeException.class)
    public ResultBudget budget(BizObject bizObject, PayMargin payMargin, String billCode, String billAction) throws Exception {
        try {
            log.error("budget...............");
            // 审批流提交动作匹配预算动作
            String action;
            ...
            // 拼装报文
            Map<String, Object> jo = initPaymarginBill(bizObject, payMargin, BudgetDirect.ADD.getIndex(), billCode, preemptionOrExecFlag, billAction);
    ...
    @Override
    @Transactional(rollbackFor = RuntimeException.class)
    public ResultBudget releaseBudget(BizObject bizObject, PayMargin payMargin, String billCode, String billAction) throws Exception {
        try {
            log.error("releaseBudget...............");
            if (payMargin.getIsOccupyBudget() != null && payMargin.getIsOccupyBudget() == OccupyBudget.UnOccupy.getValue()) {
                return new ResultBudget(OccupyBudget.UnOccupy.getValue(), false);
            }
            String action;
            // 查询是否开启预算开关
            ...
            String preemptionOrExecFlag = cmpBudgetCommonManagerService.PRE;
            // 生成
            Map bill = initPaymarginBill(bizObject, payMargin, BudgetDirect.REDUCE.getIndex(), billCode, preemptionOrExecFlag, billAction);
            ControlDetailVO[] controlDetailVOS = new ControlDetailVO[1];
    ...
    @Override
    @Transactional(rollbackFor = RuntimeException.class)
    public void executeSubmitDelete(PayMargin payMargin) throws Exception {
        ...
        ResultBudget resultBudget = releaseBudget(payMargin, payMarginNew, IBillNumConstant.CMP_PAYMARGIN, BillAction.CANCEL_SUBMIT);
    ...
    @Override
    @Transactional(rollbackFor = RuntimeException.class)
    public boolean budgetSuccess(PayMargin bill_b, boolean deleteBudget, boolean checkResult) throws Exception {
        PayMargin payMargin = MetaDaoHelper.findById(PayMargin.ENTITY_NAME, bill_b.getId());
        ...
        // 执行
        ResultBudget resultBudget = gcExecuteTrueAudit(payMargin, bill_b, IBillNumConstant.CMP_PAYMARGIN, BillAction.APPROVE_PASS, deleteBudget,checkResult);
     ...
     * @param billCode
     */
    private Map<String, Object> initPaymarginBill(BizObject bizObject, PayMargin payMargin, int addOrReduce, String billCode, String preemptionOrExecFlag, String billAction) throws Exception {
        Map<String, Object> jo = new HashMap<>();
        jo.putAll(bizObject);
        //初始化
        jo.put("signature", null);// 签名字段不传，且包含特殊字符可能被预算中台拦截
        jo.put("serviceCode", IServicecodeConstant.PAYMARGIN);
        ...
        // 特征字段拼接 例：cmpBudgetCommonManagerService.CHARACTERDEF_test0919
    ...
    public String queryBudgetDetail(CtmJSONObject json) {
        try {
            ...
            PayMargin payMargin = MetaDaoHelper.findById(PayMargin.ENTITY_NAME, id);
            String preemptionOrExecFlag = "";
            String billAction = BillAction.SUBMIT;
            if (payMargin.getIsOccupyBudget() != null && payMargin.getIsOccupyBudget() == OccupyBudget.PreSuccess.getValue()) {
                preemptionOrExecFlag = cmpBudgetCommonManagerService.PRE;
            } else if (payMargin.getIsOccupyBudget() != null && payMargin.getIsOccupyBudget() == OccupyBudget.ActualSuccess.getValue()) {
                preemptionOrExecFlag = cmpBudgetCommonManagerService.IMPLEMENT;
            } else {
                billAction = BillAction.QUERY;
            }
            String billCode = IBillNumConstant.CMP_PAYMARGIN;
            int addOrReduce = 0;//新增
            Map[] bills = new Map[1];
            Map jo = initPaymarginBill(payMargin, payMargin, addOrReduce, billCode, preemptionOrExecFlag, billAction);
            bills[0] = jo;
            ...
            if (ICmpConstant.REQUEST_SUCCESS_STATUS_CODE.equals(result.getString("code"))) {
                ...
                throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-100236"),result.getString("message"));
            }
    ...
    public String budgetCheckNew(List<BizObject> bizObjects, String billCode, String billAction) {
        try {
            ...
            for (BizObject bizObject : bizObjects) {
                ...
                PayMargin oldBill = MetaDaoHelper.findById(PayMargin.ENTITY_NAME, bizObject.getId(), 2);
            ...
            log.error("execute ", e);
            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-100234"),InternationalUtils.getMessageWithDefault("UID:P_GRM-BE_18113A5204B801BF", com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_21B2A4200540044E", "预算系统返回:") /* "预算系统返回:" */) /* "预算系统返回:" */ + e.getMessage());
        }
    }
     ...
     */
    @Override
    public void executeSubmit(PayMargin payMargin) throws Exception {
        Short budgeted = payMargin.getIsOccupyBudget();
        // 已经预占仍要预占或者已经实占，直接跳过不执行了
        if (budgeted != null && ((budgeted != OccupyBudget.UnOccupy.getValue()))) {
            log.error("executeSubmit，已经预占仍要预占或者已经实占，直接跳过不执行了");
            return;
        }
        ResultBudget resultBudget = budget(payMargin, payMargin, IBillNumConstant.CMP_PAYMARGIN, BillAction.SUBMIT);
        if (resultBudget.isSuccess()) {
     ...
     */
    @Override
    public void executeAudit(PayMargin payMargin) throws Exception {
        /**
        ...
        PayMargin update = new PayMargin();
        //结算状态为已结算补单/空时
        if (payMarginNew.getSettlestatus() == null || payMarginNew.getSettlestatus() == FundSettleStatus.SettlementSupplement.getValue()) {
            // 已经预占仍要预占或者已经实占，直接跳过不执行了
            if (budgeted != null && ((budgeted == OccupyBudget.ActualSuccess.getValue()))) {
                return;
            }
            // 1. 提交时占预占，是否占预算更新为预占成功；
            ResultBudget resultBudget = budget(payMarginNew, payMarginNew, IBillNumConstant.CMP_PAYMARGIN, BillAction.SUBMIT);
            if (resultBudget.isSuccess()) {
                update.setId(payMargin.getId());
                update.setIsOccupyBudget(OccupyBudget.PreSuccess.getValue());
                update.setEntityStatus(EntityStatus.Update);
                ...
                if (budgetSuccess(payMarginNew, true,true)) {
                    update.setId(payMargin.getId());
        ...
        } else if (FundSettleStatus.WaitSettle.getValue() == payMarginNew.getSettlestatus()) {//结算状态为待结算时
            //预占预算
            //占用成功后更新是否占预算为预占成功
    ...
    @Override
    public void reMatchBudget(PayMargin newBill) throws Exception {
        //1.判断是否是预占成功，如果是删除旧的预占，新增新的预占
        //2.判断是否是预占成功，如果否直接进行新的预占
        PayMargin oldBill = MetaDaoHelper.findById(PayMargin.ENTITY_NAME, newBill.getId(), 2);
        ...
        if (budgeted == null || budgeted == OccupyBudget.UnOccupy.getValue()) {
            ...
            ResultBudget releaseBudget = releaseBudget(oldBill, oldBill, IBillNumConstant.CMP_PAYMARGIN, BillAction.CANCEL_SUBMIT);
            if (releaseBudget.isSuccess()) {
                log.error("ReMatchBudget ，删除预算成功");
                newBill.setIsOccupyBudget(OccupyBudget.UnOccupy.getValue());
                ResultBudget resultBudget = budget(newBill, newBill, IBillNumConstant.CMP_PAYMARGIN, BillAction.SUBMIT);
                if (resultBudget.isSuccess()) {
