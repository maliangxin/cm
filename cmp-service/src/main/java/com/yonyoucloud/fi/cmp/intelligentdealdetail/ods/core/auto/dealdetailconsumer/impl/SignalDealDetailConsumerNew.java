...
import com.yonyoucloud.fi.cmp.intelligentdealdetail.ods.core.auto.dealdetailmanage.dealdetailhandler.match.SignalBankDealDetailMatchChainImplNew;
...
@Service
@Slf4j
public class SignalDealDetailConsumerNew extends DefaultDealDetailConsumer{
    @Resource
    ...
    public Map<String, List<BankReconciliation>> executeDedulication(List<BankReconciliation> bankReconciliations) {
        ...
        if(CollectionUtils.isEmpty(resultMap)){
            ...
            throw new BankDealDetailException(com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_21B2A420054005C9", "业务去重后待处理流水为空，流程结束") /* "业务去重后待处理流水为空，流程结束" */);
        }
...
//            throw new BankDealDetailException("重复数据，新增/导入失败！");
//        }
    ...
    public void callBankDetailManageAccess(List<BankDealDetailWrapper> bankDealDetailProcessList, List<BankReconciliation> updateBankReconciliationList,String traceid,String requestSeqNo) {
        ...
        bankDealDetailManageAccess.bankDealDetailManageAccess(bankDealDetailProcessList,updateBankReconciliationList, SignalBankDealDetailMatchChainImplNew.get(),traceid,requestSeqNo);
