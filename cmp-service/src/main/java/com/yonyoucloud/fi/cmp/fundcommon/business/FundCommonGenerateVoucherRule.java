...
@Component
public class FundCommonGenerateVoucherRule extends AbstractCommonRule {
...
//        }
        for (BizObject bizObject : bills) {
            ...
            int i= 0;
            boolean generatevoucherflag =false;
            if (FundCollection.ENTITY_NAME.equals(billContext.getFullname())) {
                ...
                commonFund.put("_entityName", FundCollection.ENTITY_NAME);
                List<FundCollection_b> fundCollectionBList = commonFund.get("FundCollection_b");
                for (FundCollection_b fundCollection_b : fundCollectionBList) {
                    ...
                    if (caobject == CaObject.Customer.getValue()) {
                        fundCollection_b.put(IBussinessConstant.CUSTOMER, fundCollection_b.get("oppositeobjectid"));
                    } else if (caobject == CaObject.Supplier.getValue()) {
                        fundCollection_b.put(IBussinessConstant.SUPPLIER, fundCollection_b.get("oppositeobjectid"));
                    } else if (caobject == CaObject.Employee.getValue()) {
                        fundCollection_b.put(IBussinessConstant.EMPLOYEE, fundCollection_b.get("oppositeobjectid"));
                    } else if (caobject == CaObject.CapBizObj.getValue()) {
                        fundCollection_b.put(IBussinessConstant.FUNDBUSINOBJ, fundCollection_b.get("oppositeobjectid"));
                    }
                    //计算表体金额为0的数量；
                    if(fundCollection_b.getOriSum().compareTo(BigDecimal.ZERO)==0){
                        i++;
                    }
                }
                //表体都为0，代表全部不传结算
                if(fundCollectionBList.size()==i) {
                    generatevoucherflag = true;
                }
            }
            if (FundPayment.ENTITY_NAME.equals(billContext.getFullname())) {
                ...
                commonFund.put("_entityName", FundPayment.ENTITY_NAME);
                List<FundPayment_b> FundPaymentBList = commonFund.get("FundPayment_b");
                for (FundPayment_b fundPayment_b : FundPaymentBList) {
                    short caobject = fundPayment_b.getCaobject().getValue();
                    if (caobject == CaObject.Customer.getValue()) {
                        fundPayment_b.put(IBussinessConstant.CUSTOMER, fundPayment_b.get("oppositeobjectid"));
                    } else if (caobject == CaObject.Supplier.getValue()) {
                        fundPayment_b.put(IBussinessConstant.SUPPLIER, fundPayment_b.get("oppositeobjectid"));
                    } else if (caobject == CaObject.Employee.getValue()) {
                        fundPayment_b.put(IBussinessConstant.EMPLOYEE, fundPayment_b.get("oppositeobjectid"));
                    } else if (caobject == CaObject.CapBizObj.getValue()) {
                        fundPayment_b.put(IBussinessConstant.FUNDBUSINOBJ, fundPayment_b.get("oppositeobjectid"));
                    }
                    //计算表体金额为0的数量；
                    if(fundPayment_b.getOriSum().compareTo(BigDecimal.ZERO)==0){
                        i++;
                    }
                }
                //表体都为0，代表全部不传结算，审批通过就传凭证
                if(FundPaymentBList.size()==i){
                    generatevoucherflag =true;
                }
            }
            if (isSettleSuccessToPost && !generatevoucherflag){
                continue;
            }
            //非结算成功过账
            commonFund.set("settleSuccessPost",0);
            ...
            if (CmpCommonUtil.getNewFiFlag()) {
                ...
                if (generateResult.getInteger("code") == 0 && !generateResult.getBoolean("dealSucceed")) {
                    throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-101567"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_17FE8C54041804C2","发送会计平台失败：") /* "发送会计平台失败：" */ + generateResult.get("message"));
                }
            ...
            public void afterCommit() {
                executorServicePool.getThreadPoolExecutor().submit(()->{
                    try {
                        ...
                        logparam.put("exception", e.getStackTrace());
                        AppContext.getBean(CTMCMPBusinessLogService.class).saveBusinessLog(logparam, bizObject.get(CODE), "生成事项失败日志", IServicecodeConstant.FUNDPAYMENT, "生成事项失败日志", "生成事项失败日志");//@notranslate
                        throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-100354"),InternationalUtils.getMessageWithDefault("UID:P_CM-BE_17FE8C54041805CC",com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_21B2A4200540060C", "生成凭证失败！请检查数据！") /* "生成凭证失败！请检查数据！" */));
                    }
                });
            }
        });
