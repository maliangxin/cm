...
public class DailyCompute {
     ...
     * @throws Exception
     */
    public static Map<String, SettlementDetail> imitateDailyCompute(String accentity, String currency, String cashaccount, String bankaccount, String auditstatus, String issettle, Date endDate) throws Exception {
        return doImitateDailyCompute(accentity, currency, cashaccount, bankaccount, auditstatus, issettle, endDate, endDate);
    }
...
//                        settlementDetail.setAccountdate(endDate);
        ...
        if (null!= issettle && "1".equals(issettle)&&maxDateSettlement!=null) {
            ...
            QueryConditionGroup conditionJournalXZ = getQueryConditionGroupForJournal(accentity, currency, cashaccount, bankaccount);
            ...
            QueryConditionGroup conditionJournalXZ_L = getQueryConditionGroupForJournal(accentity, currency, cashaccount, bankaccount);
        ...
        for (Map<String, Object> mapJ : queryJournal) {
            ...
            if (settlementDetail != null) {
                ...
                settlementDetail.setAccentity(journal.getParentAccentity());
    ...
    private static QueryConditionGroup getQueryConditionGroupForJournal(String parentAccentity, String currency, String cashaccount,
                                                                        String bankaccount) {
        QueryConditionGroup condition = new QueryConditionGroup();
        if (!StringUtils.isEmpty(cashaccount) && !StringUtils.isEmpty(bankaccount)) {
        ...
        if (!StringUtils.isEmpty(parentAccentity)) {
            condition.addCondition(QueryConditionGroup.and(QueryCondition.name("parentAccentity").eq(parentAccentity)));
        }
        if (!StringUtils.isEmpty(currency) ) {
            condition.addCondition(QueryConditionGroup.and(QueryCondition.name("currency").in((Object) currency.split(","))));
        }
