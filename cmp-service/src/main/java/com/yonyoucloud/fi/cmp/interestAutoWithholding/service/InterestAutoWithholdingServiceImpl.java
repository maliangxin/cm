...
public class InterestAutoWithholdingServiceImpl implements InterestAutoWithholdingService {
    ...
    @Override
    public Map<String, Object> interestAutoWithholding(String logId, String tenant,String accentity,String bankType,String bankaccount,String currency) {
        ...
        executorServicePool.getThreadPoolExecutor().submit(() -> {
            ...
            try {
                ymsLock = JedisLockUtils.lockBillWithOutTrace(tenant_id);
                if (null==ymsLock) {
                    ...
                    throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-101336"),"receiveMarginAutoTask executorServicePool lockBillWithOutTrace");
                }else {
                    ...
                    if(!"".equals(bankType)) {
                        conditionGroup.addCondition(QueryConditionGroup.and(QueryCondition.name("bankType").eq(bankType)));
                    }
                    if(!"".equals(bankaccount)) {
                        conditionGroup.addCondition(QueryConditionGroup.and(QueryCondition.name("bankaccount").eq(bankaccount)));
                    }
                    if(!"".equals(currency)) {
                        conditionGroup.addCondition(QueryConditionGroup.and(QueryCondition.name("currency").eq(currency)));
                    }
                    schema.addCondition(conditionGroup);
                    //查询银行账户利息预提数据
//                    List<AccrualsWithholding> accrualsWithholdinges = MetaDaoHelper.queryObject(AccrualsWithholding.ENTITY_NAME, schema, null);
                    List<AccrualsWithholding> accrualsWithholdinges = null;
                    if (accrualsWithholdinges != null && !accrualsWithholdinges.isEmpty()) {
                        //利息测算
                        JsonNode result = this.calculate(accrualsWithholdinges);
                        //预提
                        ArrayNode rows = result.withArray("accrualsWithholdinges");
                        Json json = new Json(rows.toString());
                        List<AccrualsWithholding> accrualsWithholdingeList = Objectlizer.decode(json, AccrualsWithholding.ENTITY_NAME);
                        //调用预提记录查询方法
                        this.tWithholding(accrualsWithholdingeList);
                    }
    ...
    public JsonNode calculate(List<AccrualsWithholding> accrualsWithholdinges) throws Exception {
        ...
        //遍历账户信息 进行校验与测算
        circulateProcess(accrualsWithholdinges,accrualsWithholdingList);
    ...
    @Override
    @Transactional(rollbackFor = RuntimeException.class)
    public void tWithholding(List<AccrualsWithholding> accrualsWithholdinges) throws Exception {
        //遍历账户信息 进行校验与测算
        tWithholdingProcess(accrualsWithholdinges);
    }
     ...
     */
    private void circulateProcess(List<AccrualsWithholding> accrualsWithholdinges,List<AccrualsWithholding> accrualsWithholdingList) {
        for (BizObject bizObject : accrualsWithholdinges) {
            ...
            try {
                //校验是否为启用银行账户
                if(!getEnterpriseBankAcctVO(accrualsWithholding.getBankaccount(), account,currencyName)){
                    continue;
                }
...
//                }
                ...
                if(ValueUtils.isNotEmptyObj(withholdingRuleSetting) && ValueUtils.isNotEmptyObj(withholdingRuleSetting.getVersion()) ){
                ...
                //计算利息与组装测算表数据
                //todo 此处报空指针   interestRateSettingHistoryList为 null
     ...
     * @throws Exception
     */
    private boolean getEnterpriseBankAcctVO(String bankAccount, String account,String currencyName) throws Exception {
...
//        conditionGroup.appendCondition(QueryCondition.name("startDate").egt(accrualsWithholding.getAccruedStartDate()));
    ...
    private void createInterest(List<InterestRateSettingHistory> interestRateSettingHistoryList, WithholdingRuleSetting withholdingRuleSetting,
        ...
        if(CollectionUtils.isNotEmpty(agreeIRSettingHistories)){
            ...
            for(AgreeIRSettingHistory agreeIRSettingHistory : agreeIRSettingHistories){
                ...
                for(int j = 0; j < agreeIRSettingGradeHistories.size() ; j++){
                    ...
                    switch (agreeIRSettingGradeHistory.getIrtype()){
                        ...
                        case 2:
                            ...
                            interestRateItem.setReferenceRate(agreeIRSettingGradeHistory.getInterestrate());
                ...
                for(DiaryItem diaryItem :interestRecordList ){
                    ...
                    interestCalculation.setDepositInterest(diaryItem.getAccrualInterest().setScale(moneydigit,moneyrount));
                    ...
                    depositAgreementCalculate=depositAgreementCalculate.add(diaryItem.getAccrualInterest()).setScale(moneydigit,moneyrount);
                }
        ...
        // 协定测算 本币
        accrualsWithholding.setCurrencyDepositAgreementCalculate(CmpExchangeRateUtils.getExchangeRateAndAmountCalResult(exchangerateOps, exchangerate, depositAgreementCalculate, null)
...
//        Integer natMoneyrount = natCurrencyDTO.getMoneyrount();
        ...
        accrualsWithholding.setCurrencyDepositInterestCalculate(CmpExchangeRateUtils.getExchangeRateAndAmountCalResult(exchangerateOps, exchangerate, calculateDepositAmounnt, null)
                .setScale(natMoneydigit, natMoneyrount));
        //透支利息测算（本币）
        accrualsWithholding.setCurrencyOverdraftInterestCalculate(CmpExchangeRateUtils.getExchangeRateAndAmountCalResult(exchangerateOps, exchangerate, calculateOverdraftAmount, null)
                .setScale(natMoneydigit, natMoneyrount));
    ...
    private Map createInterestCalculation(List<AccountRealtimeBalance> accountRealtimeBalanceList, List<InterestRateSettingHistory> interestRateSettingHistoryList,
        ...
        for (int i = 0; i < accountRealtimeBalanceList.size(); i++) {
            ...
            if (i + 1 < accountRealtimeBalanceList.size()) {
                ...
                InterestCalculation interestCalculationNext = new InterestCalculation();
    ...
    private BigDecimal getInterest(InterestCalculation interestCalculation, BigDecimal interestBaseAmount, CurrencyTenantDTO currencyDTO) {
        ...
        //计息积数*存款利率/计息天数
        return interestBaseAmount.multiply(annualInterestRate)
                .divide(new BigDecimal(interestDays),6, roundingMode).setScale(currencyDTO.getMoneydigit(), RoundingMode.HALF_UP);
    }
    ...
    private BigDecimal getOverdraftInterest(InterestCalculation interestCalculation, BigDecimal interestOverdraftBaseAmount,
        ...
        //计息积数*存款利率/计息天数
        return interestOverdraftBaseAmount.multiply(overdraftRate)
                .divide(new BigDecimal(interestDays),6, roundingMode).setScale(currencyDTO.getMoneydigit(), RoundingMode.HALF_UP);
    }

    private void tWithholdingProcess(List<AccrualsWithholding> accrualsWithholdinges) {
...
//                }
                if(!checkWithholding(accrualsWithholding, withholdingRuleSetting)){
                    continue;
                }
                //一系列校验
                // 此处有校验
...
//        }
    ...
    public void insertOrUpdate(List<InterestCalculation> interestCalculationList, WithholdingRuleSetting withholdingRuleSetting,
        ...
        //生成凭证
        cmpVoucherService.generateVoucherWithResult(accrualsWithholding);
        // 有校验
...
//            }
    ...
    private boolean checkWithholding(AccrualsWithholding accrualsWithholding, WithholdingRuleSetting withholdingRuleSetting) throws Exception {
    ...
    public static Date convertLocalDateToDate(LocalDate localDate) {
        ...
        return DateUtil.localDateTime2Date(localDate.atStartOfDay());
    }
