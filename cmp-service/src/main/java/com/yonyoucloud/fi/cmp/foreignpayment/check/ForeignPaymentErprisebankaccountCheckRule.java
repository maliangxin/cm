...
@Component
public class ForeignPaymentErprisebankaccountCheckRule extends AbstractCommonRule {
    ...
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> paramMap) throws Exception {
        ...
        if (!"paymenterprisebankaccount_account".equals(item.get("key"))) {
            return new RuleExecuteResult();
        }
        BizObject bill = getBills(billContext, paramMap).get(0);
        //判断结算方式是否为空 为空则直接返回 不对是否直联字段赋值
        if (!ObjectUtils.isEmpty(bill.get("settlemode"))) {
