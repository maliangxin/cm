...
import com.yonyoucloud.fi.cmp.intelligentdealdetail.converts.DictColumn;
...
@Data
@YMSEntity
@YMSTable(tableName = ODSSql.CMP_BANKDEALDETAIL_ODS)
public class BankDealDetailODSModel extends BaseEntity{
    @YMSId
    @YMSColumn(name = "id")
    @YMSGeneratedValue(domain = "ctm-cmp")
    @DictColumn(detailField= "id",reconciliationField = "id")
    private String id;
    ...
    private Long mainid;//银行对账单组件明细mainid',
    ...
    private String resp_service_seq_no;// '银企联响应流水号',
    @YMSColumn(name = "request_seq_no")
    private String request_seq_no ;//'请求流水号',
    @YMSColumn(name = "accesschannel")
    private int accesschannel ;//'接入ods渠道',
    @YMSColumn(name = "processstatus")
    private int processstatus ;// '请求状态1:未处理 2:开始处理 3：无需处理(业务已防重)4:辨识匹配处理中 5：已挂起 6:辨识匹配处理成功 7：辨识匹配处理失败 8：无需流程处理 9：流程处理中 10：流程处理已挂起 11:流程处理成功 12：流程处理失败',
    @YMSColumn(name = "retrycount")
    private int retrycount ;// '消费失败后重试次数，最多三次',
    @YMSColumn(name = "tran_date")
    private String tran_date;
    @YMSColumn(name = "tran_time")
    private String tran_time;
    @YMSColumn(name = "dc_flag")
    private Short dc_flag;
    ...
    @YMSColumn(name = "bankaccount")
    private String bankaccount;
    @YMSColumn(name = "currency")
    ...
    public boolean equals(Object o) {
        ...
        BankDealDetailODSModel that = (BankDealDetailODSModel) o;
