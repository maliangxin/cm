...
@Service
@Transactional
public class StwbCollectionBillServiceImpl extends StwbBillCommonService implements IFundCollectionPushStwbBillService {
    ...
    // 资金收款单子表
    private static final String CMP_FUNDCOLLECTION_B = "cmp_fundcollection_b";
    ...
    public void pushBill(List<BizObject> billList, boolean bCheck) throws Exception {
        ...
        if (billList == null) {
            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102544"), com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_17FE8C5404180797", "推送单据不可为空") /* "推送单据不可为空" */);
        }
        FundCollection fundCollection = (FundCollection) billList.get(0);
        FundCollection fund = (FundCollection) fundCollection.clone();
        ...
        if (bCheck) {
            ...
            List<FundCollection_b> fundCollectionSubList = new ArrayList<>();
            for (FundCollection_b fundCollectionB : fundCollection_bs) {
                if (!EntityStatus.Delete.equals(fundCollectionB.getEntityStatus())) {
                    fundCollectionSubList.add(fundCollectionB);
                }
            }
            //fundCollection.set("FundCollection_b",fundCollectionSubList);
            fund.set("FundCollection_b", fundCollectionSubList);
        }
        builtSystem(fund, StwbBillBuilder.builderFundCollection(fundCollection, params), CMP_FUNDCOLLECTION_B);
    }
    ...
    public void pushBillSimple(BizObject bizobject) throws Exception {
        FundCollection fundCollection = (FundCollection) bizobject;
        FundCollection collection = (FundCollection) fundCollection.clone();
        //资金收付结算是否简强
        boolean enableSimplify = settleParamPubQueryService.simplifyEnable();
        ...
        if (enableSimplify && settleflagBool) {
            ...
            List<FundCollection_b> pushsubList = new ArrayList<>();
            List<String> topushList = new ArrayList<>();
            if (!collection.FundCollection_b().isEmpty()) {
                List<FundCollection_b> subList = collection.FundCollection_b();
                for (FundCollection_b item : subList) {
                ...
                collection.setFundCollection_b(pushsubList);
            }
            builtSystem(collection, StwbBillBuilder.builderSettleMap(collection, "cmpfundcollectionToSimpleSettle",
                    "ctm-cmp.cmp_fundcollection", null, topushList), FundCollection.ENTITY_NAME, collection.getCode(), detailIdMapPubts);
        }
    }
    ...
    public void deleteBill(List<BizObject> billList) {
        ...
        ctmJSONObject.put("fundCollection", fundCollection);
        try {
            QuerySettledDetailModel querySettledDetailModel = new QuerySettledDetailModel();
            querySettledDetailModel.setBusinessId(fundCollection.getId().toString());
            querySettledDetailModel.setWdataorigin(8);// 来源业务系统，现金管理
            List detailsIds = new ArrayList();
            List<FundCollection_b> listb = fundCollection.FundCollection_b();
            for (FundCollection_b bill_b : listb) {
            ...
            if (enableSimplify) {
                ...
                idList.add(fundCollection.getId());
                unifiedSettlePubService.deleteUnifiedSettle(FundCollection.ENTITY_NAME, idList.toArray(new String[0]), PushOrder.FIRST);
            }
            for (FundCollection_b bill_b : listb) {
                //结算状态为结算止付 是否委托驳回为是 表示已经被委托拒绝 不进行更新
                if (bill_b.getFundSettlestatus().getValue() != FundSettleStatus.SettleFailed.getValue()
                        && (bill_b.getEntrustReject() == null || bill_b.getEntrustReject() != 1)) {
                    Map<String, Object> params = new HashMap<>();
                    FundCollection_b fundCollection_b = new FundCollection_b();
                    fundCollection_b.setId(bill_b.getId());
                    fundCollection_b.setTransNumber(null);
                    ...
                    params.put("tableName", CMP_FUNDCOLLECTION_B);// 资金收付款单表名
                    ...
                    if (bill_b.getFundSettlestatus().getValue() != FundSettleStatus.SettlementSupplement.getValue()
                        ...
                        fundCollection_b.setFundSettlestatus(FundSettleStatus.WaitSettle);
                        ...
                        fundCollection_b.setSettlesuccessSum(BigDecimal.ZERO);
                    }
            ...
            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102545"), com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_17FE8C5404180796", "删除结算单据失败：") /* "删除结算单据失败：" */ + e.getMessage());
        } finally {
//            saveBusinessLog(fundCollection, ctmJSONObject);
        }
    }
    ...
    public void saveBusinessLog(BizObject bizObject, CtmJSONObject ctmJSONObject) {
        try {
            ctmcmpBusinessLogService.saveBusinessLog(
                    ...
                    IMsgConstant.FUND_COLLECTION_DELETE_DATA_SETTLES,
