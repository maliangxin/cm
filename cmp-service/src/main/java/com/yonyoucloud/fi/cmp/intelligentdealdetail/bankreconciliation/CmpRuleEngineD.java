...
@Component
@Scope("prototype")
public class CmpRuleEngineD extends AbstractCmpRuleEngine {
    ...
    public void loadRule(boolean isReturnMsg) throws Exception {
        ...
        if (executeRuleMap.size() == 0) {
            ...
            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-102140"),noRuleMsg);
        }
    }
    ...
    public void executeRule(List<BankReconciliation> bankrecList, boolean isReturnMsg) throws Exception {
        try {
            ...
            // 先做条件处理，减少查询
            // TODO 此处查询数据库了，新流程此时还没入库，需要用bankrecList直接转换尝试
//            super.querySourcesValue(bankrecList, sourcesMap);
            for (BankReconciliation bankReconciliation : bankrecList) {
                sourcesMap.put(Long.parseLong(bankReconciliation.getId().toString()), bankReconciliation);
            }
            // 获取执行rule
            TreeMap<Integer, TargetRuleInfoDto> executeRuleMap = getExecuteRuleMap();
