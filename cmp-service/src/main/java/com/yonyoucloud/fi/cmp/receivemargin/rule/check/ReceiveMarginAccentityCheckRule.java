package com.yonyoucloud.fi.cmp.receivemargin.rule.check;
...
/**
 * 收到保证金 会计主体，交易类型等默认值查询校验
 */
@Slf4j
@RequiredArgsConstructor
@Component
public class ReceiveMarginAccentityCheckRule extends AbstractCommonRule {
...
//    OrgRpcService orgRpcService;
    ...
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> paramMap) throws Exception {
        ...
        if (Objects.isNull(bdBillType)) {
            //todo 多语
            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-101209"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_18EC6C5A05080004", "查询收到保证金台账管理单据类型失败！") /* "查询收到保证金台账管理单据类型失败" */);
        }
        try {
...
//                bill.set("exchRate", 1);
        ...
        if (defaultExchangeRateType != null && defaultExchangeRateType.get("id") != null) {
            ...
            bill.set("exchangeratetype_code", defaultExchangeRateType.get("code"));
        }
        // 设置交易类型
...
//        }
        ...
        //设置是否期初数据：否
        bill.set("initflag", 0);
        if (bill.get("settleflag").equals((short)1)) {
            bill.set("settleflag", 1);
        } else {
            //设置是否结算标识为：否
            bill.set("settleflag", 0);
        }
        //设置是否结算标识为：否
//        bill.set("settleflag", 0);
        if (bill.get("conversionmarginflag").equals((short)1)) {
            bill.set("conversionmarginflag", 1);
        } else {
            bill.set("conversionmarginflag", 0);
        }
//        bill.set("conversionmarginflag", 0);

        if (bill.get("autorefundflag").equals((short)1)) {
            bill.set("autorefundflag", 1);
        } else {
            bill.set("autorefundflag", 0);
        }

        if (ObjectUtils.isNotEmpty(bill.get("refundsettleflag"))) {
            if (bill.get("refundsettleflag").equals((short)1)) {
                bill.set("refundsettleflag", 1);
            } else {
                bill.set("refundsettleflag", 0);
            }
        } else {
            bill.set("refundsettleflag", 0);
        }
    ...
    private void setValueForTransType(BizObject bill) {
        ...
        try {
            ...
            conditionGroup.appendCondition(QueryCondition.name("form_id").eq(ICmpConstant.CM_CMP_RECEIVEMARGIN));
            ...
            if (CollectionUtils.isNotEmpty(list)) {
                ...
                if (!ValueUtils.isNotEmptyObj(objectMap)) {
                    //todo 多语
                    throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-101210"),com.yonyou.iuap.ucf.common.i18n.InternationalUtils.getMessageWithDefault("UID:P_CM-BE_18EC6C9C05080007", "查询收到保证金交易类型失败！请检查数据。") /* "查询收到保证金交易类型失败！请检查数据。" */);
                }
...
//            throw new CtmException(new com.yonyoucloud.fi.cmp.common.CtmErrorCode("033-502-101211"),"未获取到默认的交易类型！");
        }
    }
