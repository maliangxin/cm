package com.yonyoucloud.fi.cmp.checkmanage.refer;
...
@Slf4j
@Component
public class CheckManageTradeTypeReferFilterRule extends AbstractCommonRule {
    /**
     * 需要进行过滤的billnum集合
     */
    private static final List<String> BILLNUM_MAP = new ArrayList<>();

    public CheckManageTradeTypeReferFilterRule() {
        //保证金存入支取单
        BILLNUM_MAP.add("cmp_checkdisposalcard");
        BILLNUM_MAP.add("cmp_checkdisposallist");
    }

    @Override
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> map) throws Exception {
        log.info("支票处置交易类型过滤start#########################");
        BillDataDto billDataDto = (BillDataDto) getParam(map);
        String billnum = billDataDto.getBillnum();
        log.info("#########################billnum is:" + billnum);
        ...
        try {
            ...
            if (org.apache.commons.collections4.CollectionUtils.isNotEmpty(list)) {
                ...
                if (!com.yonyoucloud.fi.cmp.util.ValueUtils.isNotEmptyObj(objectMap)) {
                    log.error("查询支票处置交易类型失败！请检查数据。");
                }
                billTypeId = MapUtils.getString(objectMap, "id");
            }
            log.info("#########################billTypeId is:" + billTypeId);
